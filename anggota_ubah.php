...
$row = mysqli_fetch_array(mysqli_query($con, "SELECT * FROM users INNER JOIN jenis_keanggotaan ON users.id_jeniskeanggotaan = jenis_keanggotaan.id_jeniskeanggotaan INNER JOIN agama ON users.id_agm = agama.id_agm INNER JOIN jenis_kelamin ON users.id_jk = jenis_kelamin.id_jk 
	...
	<div class="form-group row">
		<div class="col-sm-6 mb-3 mb-sm-0">
			...
			<input type="text" class="form-control" name="id_jeniskeanggotaan" value="<?php echo $row['nama_jeniskeanggotaan'];?>" disabled>
		</div>
		<div class="col-sm-6 mb-3 mb-sm-0">
			<label>Jabatan</label>
		...
		<div class="col-sm-6 mb-3 mb-sm-0">
			...
			<select class="form-control" name="id_jk" id="jk" >
				...
				while($r=mysqli_fetch_array($tampil)) {
					?>
					<option class="<?php echo $row['id_jk'] ?>"  value="<?php echo "$r[id_jk]";?>" <?php echo $r['id_jk'] == $row['id_jk'] ? 'selected="selected"' : '' ?>><?php echo $r['nama_jk']; ?></option>
					<?php
				}
				?>
			</select>
	...
	<div class="form-group row">
		...
		<div class="col-sm-6 mb-3 mb-sm-0">
			<label>Pendidikan Terakhir</label>
			<input type="date" class="form-control" name="tgl_bergabung" value="<?php echo $row['tgl_bergabung'];?>" disabled>
		</div>
	</div>
	<div class="form-group row">
		<div class="col-sm-6 mb-3 mb-sm-0">
