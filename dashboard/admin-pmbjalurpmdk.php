    ...
    <div class="container-fluid">
      ...
      <div class="card">
        <div class="card-header <?php echo "$iden[color]";?>">
          <h3 class="card-title"><i class="fa fa-list-ol"></i> <b>DATA PENDAFTAR PMB JALUR PMDK</b></h3>
          ...
          <div class="card-tools">
            ...
            <button type="button" class="btn btn-xs btn-tool btn-light dropdown-toggle dropdown-icon" data-toggle="dropdown">
              ...
              <div class="dropdown-menu small" role="menu">
                <a class="dropdown-item small" onclick="window.location.href='pmbjalurpmdk'" type="button"><i class="fa fa-angle-right"></i> Jalur PMDK</a>
                <a class="dropdown-item small" onclick="window.location.href='pmbgelombang1'" type="button"><i class="fa fa-angle-right"></i> Gelombang 1</a>
                <a class="dropdown-item small" onclick="window.location.href='pmbgelombang2'" type="button"><i class="fa fa-angle-right"></i> Gelombang 2</a>
              </div>
            </button>
            <button type="button"  onclick="window.location.href='excel/pmbjalurpmdk.php'" class="btn btn-xs btn-tool btn-light"  data-toggle="tooltip" title="Ekspor Data Ke Excel"><i class="fa fa-file-excel"></i> Ekspor Excel</button>
              ...
              <script type="text/javascript">
                $(document).ready(function(){
                  ...
                  var table = $('#tabel_kta').DataTable( {
                    ...
                    "processing": true,
                    "serverSide": true,
                    "ajax": 'serverside/pmb_jalurpmdk.php', 
...

                    {
                      ...
                      "render": function(data, type, full) {  
                       return '<center><div class="btn-group"><button type="button" style="width:25px;" class="btn btn-xs bg-navy resetdata" data-id="'+data+'" data-toggle="tooltip" title="Reset Password"><i class="fa fa-key"></i></button><button type="button" style="width:25px;" class="btn btn-xs bg-primary verifikasidata" data-id="'+data+'" data-toggle="tooltip" title="verifikasi Data"><i class="fa fa-check"></i></button><a type="button" style="width:25px;" onclick="window.location.href=\'pmbjalurpmdkubah?id='+data+'\'" class="btn btn-xs bg-dark" data-toggle="tooltip" title="Ubah Data"><i class="fa fa-edit"></i> <?php echo $a; ?></a><button type="button" style="width:25px;" class="btn btn-xs bg-red hapusdata" data-id="'+data+'" data-toggle="tooltip" title="Hapus Data"><i class="fa fa-trash"></i></button></div></center>';
                     } },
                     ...
                     { "targets": 6, "className": 'center'},
                     {
                      ...
                      "render": function (data, type, row) {
                        if (data == '') {
                          return "<label class='badge bg-maroon'><i class='fa fa-file-text'></i> Belum Transfer</label>";
                        }
                        else{
                          return "<label class='badge bg-info'><i class='fa fa-file-text'></i> Sudah Transfer</label>";
                        }
                      }
                    },

                    {
                      ...
                      "render": function (data, type, row) {
                        if (data == '') {
                          return "<center>Belum Unggah <i class='fa fa-cloud-upload'></i></center>";
                        }
                        else{
                          return '<center><a data-toggle=\"tooltip\" title=\"Lihat Bukti Transfer\" href=\"\" onclick=\"window.open(\'../assets/dist/img/buktitransfer/'+data+'\',\'popuppage\',\'width=500,toolbar=0,resizable=0,scrollbars=yes,height=500,top=100,left=300\');\" ><img class="img-responsive img-rounded img-thumbnail" alt="Responsive image" src="../assets/dist/img/buktitransfer/'+data+'" style="border: 1px solid #007bff;height:40px;width:40px;object-fit: cover;overflow: hidden;"></a></center>';
                        }
                      }
                    },

                    {
                      ...
                      "render": function (data, type, row) {
                        if (data == '0') {
                          return "<label class='badge bg-danger'><i class='fa fa-times-circle'></i> Belum Diverifikasi</label>";
                        }
                        else
                          return "<label class='badge bg-primary'><i class='fa fa-check-circle'></i> Diterima</label>";
                      }
                    },
                    ...
                    "render": function(data, type, row) {
                      ...
                      { "targets": 13, "className": 'center'},
                      {
                        ...
                        "render": function (data, type, row) {
                          if (data == '') {
                            return '<label class="badge bg-danger"><i class="fa fa-cloud-upload"></i> Belum Diunggah</label>';
                          }
                          else{
                           return '<a class="badge badge-success" data-toggle=\"tooltip\" title=\"Lihat Surat Penerimaan\" href=\"\" onclick=\"window.open(\'../assets/dist/img/surat/'+data+'\',\'popuppage\',\'width=420,toolbar=0,resizable=0,scrollbars=yes,height=400,top=100,left=300\');\" ><i class="fa fa-eye"></i> Lihat Surat</a>';
                         }
                       }
                     }
                     ],
...
<form action="action/pmbjalurpmdk_verifikasi.php" method="POST" class="form-horizontal" enctype="multipart/form-data">
...
<form action="action/pmbjalurpmdk_delete.php" method="GET" class="form-horizontal" enctype="multipart/form-data">
...
<form action="action/pmbjalurpmdk_reset.php" method="POST" class="form-horizontal" enctype="multipart/form-data">
