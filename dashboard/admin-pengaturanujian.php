		...
		<script type="text/javascript">
			$(function() {
				Swal.fire({
					...
					window.location = "pengaturanujian";
				});
			});
		</script>
	<?php }       
...
<div class="content-wrapper">
		...
		<div class="container-fluid">
			...
			<div class="card">
				<div class="card-header  <?php echo "$iden[color]"; ?>">
					<h3 class="card-title"><i class="fa fa-edit (alias)"></i> <b>FORM UBAH DATA BIODATA PROGRAM STUDI</b></h3>
					<center></center>
					<div class="card-tools">
						...
						<!-- /.card-header -->
						<div class="card-body">
							<form class='form-horizontal' role='form' method=POST action='action/pengaturanujian_update.php' enctype='multipart/form-data'>
								<?php
								$r=mysqli_fetch_array(mysqli_query($con, "SELECT * FROM tbl_pengaturan_ujian WHERE id_ujian ='1'"));
								?>
								<input type='hidden' class='form-control' name='id_ujian' value="<?php echo $r['id_ujian']; ?>">

								<div class="row">
									<div class="col-sm-6">
										<div class="form-group">
											<label>Nama Ujian</label>
											<input type="text" name="nama_ujian" class="form-control" value="<?php echo $r['nama_ujian']; ?>"><br/>
											<label>Nilai Minimal Kelulusan Ujian</label>
											<input type="text" name="nilai_min" class="form-control" value="<?php echo $r['nilai_min']; ?>"><br/>
											<div class="row">
												<div class="col-sm-4">
													<label>Waktu Ujian (Jam)</label>
													<input type="number" name="jam" class="form-control" value="<?php echo $r['jam']; ?>"><br/>
												</div>
												<div class="col-sm-4">
													<label>Waktu Ujian (Menit)</label>
													<input type="number" name="menit" class="form-control" value="<?php echo $r['menit']; ?>"><br/>
												</div>
												<div class="col-sm-4">
													<label>Waktu Ujian (Detik)</label>
													<input type="number" name="detik" class="form-control" value="<?php echo $r['detik']; ?>"><br/>
												</div>
											</div>
											<label>Status Ujian</label>
											<select name='link_ujian' class='form-control' id='ujian'>
												<option></option>
												<option class="<?php echo $r['link_ujian']; ?>" value="soalujian" <?php echo 'soalujian' == $r['link_ujian'] ? 'selected="selected"' : '' ?>>Aktif</option><option class="<?php echo $r['link_ujian']; ?>" value="infoujian" <?php echo 'infoujian' == $r['link_ujian'] ? 'selected="selected"' : '' ?>>Tidak Aktif</option>
											</select><br/><br/>
											<label>Link Ujian</label>
											<input type="text" class="form-control" value="<?php echo "$url"?>dashboard/<?php echo $r['link_ujian']; ?>" readonly="readonly">
									...
									<div class="col-sm-6">
										<div class="form-group">
											...
											<div class="pad">
												<textarea class="textarea" name="peraturan" style="width: 100%; height: 400px; font-size: 14px; line-height: 18px; border: 1px solid #dddddd; padding: 10px;" ><?php echo $r['peraturan']; ?></textarea>
											</div>
										</div>
									</div>
								</div>
	...
	<script>
		$( document ).ready(function() {
			$('#ujian').select2({
				placeholder: 'Pilih Status Ujian',
				language: "id"
			});
			$('#pilihan').select2({
